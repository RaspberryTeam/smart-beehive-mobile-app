...
import { ActivityIndicator, StyleSheet, Text, View, Alert, useWindowDimensions, ScrollView } from "react-native";
import { format } from "date-fns";
import { uk } from "date-fns/locale";
import { compareByDate } from "../utils/utils";
import { TouchableOpacity } from "react-native";
import { LineChart } from "react-native-chart-kit";
import { formatDataset, lineCharData } from "../utils/data";
...
const BeehiveCardInfo = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch(`http://localhost:3000/api/beehives/get-details/${beehiveId}`);
            if (!response.ok) {
